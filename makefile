...
C_SOURCES := main.c password.c mainmenu.c window_switcher.c newtrans.c RNG.c history.c settlement.c option.c registration.c config.c crypto.c logDB.c
...
$(C_EXECUTABLE): $(C_SOURCES)
	$(CC) $(CFLAGS) -o $(C_EXECUTABLE) $(C_SOURCES) `pkg-config gtk+-3.0 libglade-2.0 --cflags --libs` `pkg-config --cflags --libs libconfig` -lssl -lcrypto -lsqlite3
